...
;;jeq definitions
;;;with literal k
(define (jeq-k st k lt (lf null))
  (jmp-cond st bveq k lt lf))

;;;with reg-x
(define (jeq-x st lt (lf null))
...
(define (jneq st lt . k)
  (if (null? k)
      ...
      (jmp-cond st bvneq (car k) lt null)))

;;jlt definition
(define (jlt st lt . k)
  (if (null? k)
      (jmp-cond  st bvslt (ebpf-state-reg-x st) lt null)
      (jmp-cond st bvslt (car k) lt null)))

;;jle definition
(define (jle st lt . k)
  (if (null? k)
      (jmp-cond  st bvsle (ebpf-state-reg-x st) lt null)
      (jmp-cond st bvsle (car k) lt null)))

;;jgt definitions
;;;with literal k
(define (jgt-k st k lt (lf null))
  (jmp-cond st bvsgt k lt lf))

;;;with reg-x
(define (jgt-x st lt (lf null))
  (jmp-cond st bvsgt (ebpf-state-reg-x st) lt lf))

;;jge definitions
;;;with literal k
(define (jge-k st k lt (lf null))
  (jmp-cond st bvsge k lt lf))

;;;with reg-x
(define (jge-x st lt (lf null))
